...
public class NotificationService extends NotificationListenerService {
    ...
    private void sendMessage(){
        Intent msgrcv = new Intent("LocalMsg");
        String kakaoMiss = miss.replaceAll("[^0-9]", "");
        String kakaoTitle =title;
        String kakaoContent =content;
        Log.i(TAG, "부재중 메시지 : "+ kakaoMiss  + " 제목 : "+  kakaoTitle + " 본문 : "+ kakaoContent  + " 시간 : " + mTime);
        msgrcv.putExtra("kakaoInfo", kakaoMiss + "," +  kakaoTitle + "," + kakaoContent);
        LocalBroadcastManager.getInstance(context).sendBroadcast(msgrcv);
    }
